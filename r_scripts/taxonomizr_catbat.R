...
if (file.size(args) == 0L) {
  print("File is empty")
  args2 = gsub(".taxo.txt","_catbat_anno.xlsx",args)
  cat(NULL,file=args2)
  quit() }
file <- read.csv(args, sep='\t',header = F)
#file <- read.csv("/mnt/disk1/PROJECTS/SURPRISE/eshi_i_myshi/RESULTS/results/spades/filt_contigs/catbat/catbat_res_F350010037_L12_1.contigs.taxo.txt",sep='\t',header=F)
